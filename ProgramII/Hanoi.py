...
def a_star_branch_bound(start, goal, heuristic, neighbors):
    """
    Implementación de A* con Branch and Bound.
    ...
    """
    pq = []  # Cola de prioridad (min-heap) basada en f(n)
    heapq.heappush(pq, (heuristic(start), 0, start, []))  # (f(n), g(n), nodo, camino)
    
    best_cost = float('inf')  # Límite superior inicial (Branch and Bound)
    best_path = None
    ...
    while pq:
        ...
        # Si ya encontramos un camino mejor, descartamos
        if g >= best_cost:
            continue
        
        # Si alcanzamos la meta y es óptima, actualizamos mejor solución
        if node == goal:
            best_cost = g
            best_path = path + [node]
            continue
        
        # Marcar nodo como visitado con el menor costo g(n) conocido
...
if __name__ == "__main__":
    ...
    resultado = a_star_branch_bound(inicio, objetivo, funcion_heuristica, funcion_sucesor)
