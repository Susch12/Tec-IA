...
def funcion_heuristica(estado):
    ...
    for i in range(N, 0, -1):
        if i in torre0:
            ...
            bloqueado = index < len(torre0) - 1
            total += (2 ** index) + (5 if bloqueado else 0) 
        elif i in torre1:
            index = torre1.index(i)
            bloqueado = index < len(torre1) - 1
            total += (2 ** index) + (5 if bloqueado else 0)
        elif i in torre2:
            index = torre2.index(i)
            if index != len(torre2) - i:  # No está en la posición correcta
                total += 5
